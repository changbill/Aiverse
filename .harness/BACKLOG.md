...
- 9단계 전체 흐름 검증과 미니PC 운영 배포 검증 모두 이 환경에 브라우저 도구가 없어 실제 React UI 클릭 대신 curl 기반 API 호출로 대체 수행함(로컬·운영 도메인 모두 회원가입→업로드→충전→구매→다운로드→대시보드 결과 정상) — React 컴포넌트 자체의 렌더링·상호작용(폼 검증 메시지, 로딩 스피너, 라우팅 전환 등)은 검증되지 않았으므로 `https://aiverse-blue.vercel.app`에서 브라우저로 한 번 더 수동 확인 권장
