...
- [ ] 백엔드 Dockerfile 작성(멀티스테이지: Gradle 빌드 → JRE 실행 이미지)
- [ ] `docker-compose.yml`에 `app` 서비스 추가, MinIO S3 API 포트를 `9998:9000`으로, app 포트를 `9999:8080`으로 매핑(MinIO 콘솔 `9001:9001`은 기존 설정 유지)
