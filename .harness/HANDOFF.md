...
> **이 문서에 담지 않는 것:** 단계별 완료 요약은 여기 쓰지 않고 `STATE.md`에 한 줄로 반영한다. 여기는 세션 단위 서술만 남긴다.
...
**다음에 할 일:** PLAN.md 3단계 "카테고리·태그·콘텐츠 탐색"을 새 브랜치 전략(`feature/12-...`, 단계 전체를 한 브랜치에서 체크리스트 항목별 커밋으로 진행)으로 시작한다.

**참고사항:** 이 작업은 문서/설정 전용 변경이라 feature 브랜치 없이 master에 직접 커밋했다. `HANDOFF.md`의 과거 세션 기록(이슈 1~11)은 원래 목적(세션별 감사 추적)에 맞으므로 압축·삭제하지 않고 그대로 뒀다 — 중복 문제는 `STATE.md` 쪽이었다.
