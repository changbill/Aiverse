...
`.harness/ARCHITECTURE.md`의 "개발 방법론 — TDD"를 따른다. Entity 자체는 테스트 대상이 아니지만, Entity가 실제 스키마와 맞물리는지는 `IntegrationTestSupport`(Testcontainers MySQL) 기반 컨텍스트 로딩/Flyway `validate`로 검증되므로 Entity 설계 완료 후 반드시 컨텍스트가 기동되는지 확인한다.
