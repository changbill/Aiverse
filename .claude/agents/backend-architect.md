...
`.harness/ARCHITECTURE.md`의 "개발 방법론 — TDD"를 따른다. Entity 자체는 테스트 대상이 아니지만, Entity가 실제 스키마와 맞물리는지는 `IntegrationTestSupport`(Testcontainers MySQL) 기반 컨텍스트 로딩/Flyway `validate`로 검증된다. 통합 테스트를 TDD로 작성·수정하는 작업 중에는 해당 테스트를 실행하고, 전체 통합 스위트는 사용자 요청 시에만 실행한다 (`CLAUDE.md`의 "하네스: 테스트 실행 정책" 참조).
