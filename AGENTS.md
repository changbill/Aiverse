...
- 이슈 하나당 브랜치 하나, 커밋 하나를 원칙으로 한다. 여러 이슈를 포함하는 모듈·단계 단위 작업이라도 이슈들을 한 브랜치에 묶지 않고, 이슈별로 별도 브랜치를 만들어 구현·커밋한다.
- 한 이슈의 구현과 커밋이 끝나면 바로 master에 병합하고, 사용자에게 완료를 보고해 확인을 받은 뒤에만 다음 이슈의 브랜치를 새로 만든다.
